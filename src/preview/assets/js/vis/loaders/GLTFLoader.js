...
class GLTFParser {
  ...
  loadScene(sceneIndex) {
    ...
    for (let i = 0, il = nodeIds.length; i < il; i++) {
      pending.push(buildNodeHierarchy(nodeIds[i], scene, json, parser, i));
    }
    ...
    return Promise.all(pending).then(function () {
      ...
      function sortChildren(object) {
        if (object.children && object.children.length) {
          const children = object.children;
          const array = new Array(children.length).fill(undefined);
          let canSort = true;
          for (const child of children) {
            if (child.parentIndex === undefined) {
              ...
              array[child.parentIndex] = child;
              delete child.parentIndex;
              sortChildren(child);
            }
          }
...
function buildNodeHierarchy(nodeId, parentObject, json, parser, index) {
    ...
    .then(function (node) {
      ...
      if (nodeDef.children) {
        ...
        for (let i = 0, il = children.length; i < il; i++) {
          ...
          pending.push(buildNodeHierarchy(child, node, json, parser, i));
        }
      }
