  ...
  <div class="headerOperation-container">
    <div class="operate-history">
      <vis-icon
        ...
        @click.native="
          () => {
            $store.commit('shaderLibrary/backOffHistory');
          }
        "
      ></vis-icon>
      <vis-icon
        ...
        @click.native="
          () => {
            $store.commit('shaderLibrary/forwardHistory');
          }
        "
      ></vis-icon>
    </div>
    ...
    <div class="operate-operation">
      ...
      <vis-icon
        v-if="canUpload"
        v-tooltip.top="
          '上传着色器npm压缩包，包含js文件、package.json、preview.png预览图'
        "
...
export default {
  ...
  computed: {
    currentFloder() {
      return this.$store.getters["shaderLibrary/currentFloder"];
    },
    // 可以上传的标识
    canUpload() {
  ...
  methods: {
    chouseFile(item) {
      if (item.dir) {
        this.$store.commit("shaderLibrary/currentFloder", item);
      }
    },
    upload() {
    ...
    async fileHandler(event) {
      ...
      shaderApi
        ...
        .then((data) => {
          this.$store.commit("shaderLibrary/addChildren", data);
        })
        .finally(() => {
          this.$refs.uploadInput.value = "";
        });
    ...
    addClassify() {
      if (this.classifyName.trim()) {
        shaderApi
          ...
          .then((res) => {
            this.$store.commit("shaderLibrary/addChildren", res);
            this.classifyName = "";
